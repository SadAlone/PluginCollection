		...
		if (FMath::Abs(P0.Y - P1.Y) < 4 * RoundRadius)
		{
			...
			else
			{
				...
				*/
				...
				const FVector2D P72 = P1 + FVector2D(MinXOffset, 0.0f);
				const FVector2D P6 = P72 + FVector2D(0.0f, -2.0f * RoundRadius);
				...
				ENDrawSpline(LayerId, P4, DownTangent, P5, ForwardTangent, Params);
				ENDrawSpline(LayerId, P5, NullTangent, P6, NullTangent, Params);
				ENDrawSpline(LayerId, P6, ForwardTangent * TD, P72, -ForwardTangent * TD, Params);
			}
		}
		else
		{
